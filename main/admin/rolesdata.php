...
$result = $obj->selectextrawhereupdate(
    ...
    "status=1 and id != 1 $search $order"
);
...
while ($row = $obj->fetch_assoc($result)) {
    ...
    if (in_array(26, $permissions)) {
        $a = '<a class="btn" style="margin-right:10px;" href="editrole?hakuna=' . $row['id'] . '" >Edit</a>';
    }
    if (in_array(27, $permissions)) {
        ...
        $a .= "<a style='cursor: pointer;' class='btn btn-primary'  onclick='del(\"" . $row['id'] . "\", \"deleterole\", \"Delete Role \")' >Delete</a>";
    }
    $n[] = $a;
    $data[] = $n;
    $i++;
